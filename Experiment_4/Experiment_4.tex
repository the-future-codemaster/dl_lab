...
\end{center}
\textbf{Interpretation:} The sample images properly display various classes from the CIFAR-10 dataset, confirming successful data loading.

\vspace{5mm}
\begin{center}
    \includegraphics[width=0.75\textwidth]{train_val_accuracy.eps}
\end{center}
\textbf{Interpretation:} The fine-tuned model demonstrates rapid convergence with training accuracy reaching over 96\%, while validation accuracy plateaus at $\sim$70\%, indicating overfitting.

\vspace{5mm}
\begin{center}
    \includegraphics[width=0.75\textwidth]{train_val_loss.eps}
\end{center}
\textbf{Interpretation:} The training loss decreases sharply, but the validation loss begins to increase after epoch 4, corroborating the overfitting observed in the accuracy plot.

\vspace{5mm}
\begin{center}
    \includegraphics[width=0.75\textwidth]{confusion_matrix(1).eps}
\end{center}
\textbf{Interpretation:} The confusion matrix shows a strong diagonal, but visual similarities between classes like cat and dog lead to noticeable off-diagonal misclassifications.

\vspace{5mm}
\begin{center}
    \includegraphics[width=0.85\textwidth]{misclassified-2.eps}
\end{center}
\textbf{Interpretation:} The misclassified images reveal that the model struggles primarily with complex backgrounds and visually ambiguous instances, which is typical for standard architectures without heavy augmentation.
...
In this experiment, I explored the evolution of deep CNN architectures and practically applied Transfer Learning to classify the CIFAR-10 dataset. The key takeaways from a practical and viva perspective are:

\begin{itemize}
\item \textbf{Architectural Evolution:} I learned how CNNs evolved from the simple 7-layer LeNet to the 50-layer ResNet, and how innovations like $3\times3$ filters (VGG), multi-scale Inception modules (GoogleNet), and skip connections (ResNet) solved problems like computational bloat and vanishing gradients.
\item \textbf{Power of Transfer Learning:} Instead of training from scratch, I successfully repurposed a pre-trained VGG16 model. By freezing the base layers to retain general feature extractors (like edge detectors), I saved massive amounts of computational time.
\item \textbf{Impact of Fine-Tuning:} I observed first-hand that fine-tuning (unfreezing the deeper layers) and using the Adam optimizer were critical steps. It allowed the network to adapt its high-level features specifically to CIFAR-10, boosting the validation accuracy to $\sim$70\%.
\end{itemize}
