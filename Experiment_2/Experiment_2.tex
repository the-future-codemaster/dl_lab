...
GitHub repository:
\url{https://github.com/the-future-codemaster/dl_lab/tree/main/Experiment_2}
...
\end{center}
\textbf{Interpretation:} We get an output of 1 when both the inputs are different i.e. [0,1] or [1,0] from a XOR Gate. A Single-Layer Perceptron cannot separate the positive from the negative class because a straight line cannot separate a diagonal cross. Therefore it fails. A Multi-Layer Perceptron used backpropagation and hidden layers to change the feature space. It changes the decision boundary to curve around the inputs dynamically and solve the non-linear problem.
