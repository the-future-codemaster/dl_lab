...
Avoiding normalisation will give us features where the larger numbers will completely dominate the weight updates. The model will focus mostly on those larger features thereby ignoring the smaller ones. This will cause the training process to either extend or completely fail. A Z-score normalisation makes every single feature to have a mean of 0 and a standard deviation of 1. This balances and gives the error surface a much rounder shape. Therefore the gradient descent algorithm can take more direct and efficient steps toward the minimum, making convergence significantly faster.
...
\end{center}
\textbf{Interpretation:} For the AND gate, the only output of 1 is when both inputs are 1. The plot shows the decision boundary evolution over 8 updates across 4 epochs. Because the problem is linearly separable, the Single-Layer Perceptron correctly separates the positive class from the negative classes using a single straight line.
...
\end{center}
\textbf{Interpretation:} The OR gate evaluates to 1 if any input is 1. We observe the initial states progressing until update 5, where the boundary isolates the $[0,0]$ case entirely. The problem is linearly separable, which allows the perceptron to find the optimal hyperplane easily.
...
\end{center}
\textbf{Interpretation:} The NOT gate involves a 1D input. As shown in the plot evolution, the model only took 2 weight updates over 3 epochs to correctly draw a linear threshold separating the two logical states.
